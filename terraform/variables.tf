...
variable "my_ip_cidr" {
  description = "Your public IP in /32 CIDR notation — restricts SSH access to the bastion and Ansible controller (e.g. 203.0.113.1/32)"
  type        = string
}

variable "custom_ami_id" {
  description = "AMI ID produced by the Packer build (Amazon Linux 2023 + Docker + SSH key)"
  type        = string
}
